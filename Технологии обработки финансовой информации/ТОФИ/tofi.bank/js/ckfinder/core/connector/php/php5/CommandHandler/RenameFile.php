...
class CKFinder_Connector_CommandHandler_RenameFile extends CKFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
    ...
    protected function buildXml()
    {
        ...
        require_once(CKFinder_Connector_Utils_FileSystem::getDocumentRootPath().'/Classes/Libs/Formaters/Transliterator.php');
        $type = explode('.', $newFileName);
        $type = strtolower($type[sizeof($type)-1]);
        $filename = implode('.', explode('.', $newFileName, -1));
        $newFileName = Transliterator::Transliterate($filename, 'original', true, false).'.'.$type;
