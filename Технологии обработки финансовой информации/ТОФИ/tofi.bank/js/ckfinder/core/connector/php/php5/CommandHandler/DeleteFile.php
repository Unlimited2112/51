...
class CKFinder_Connector_CommandHandler_DeleteFile extends CKFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
    ...
    protected function buildXml()
    {
        ...
        if ($bDeleted) {
            $thumbs = new DirectoryIterator($this->_currentFolder->getThumbsServerPath());
            while($thumbs->valid()) 
            {
                $thumbname = explode('_', $thumbs->current()->getFilename(), 2);
	
                if (is_array($thumbname) and count($thumbname) > 1 and ($thumbname[1] == $fileName))
                {
                    @unlink($thumbs->current()->getPathname());
                }
